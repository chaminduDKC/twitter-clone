...
export const updateUser = async (req, res) => {
    ...
    try {
        const user = await User.findByIdAndUpdate({ clerkId: userId }, req.body, { new: true, runValidators:true });
        if (!user) return res.status(404).json({ error: "User not found" });
...
export const followUser = async (req, res) => {
    try {
        ...
        if (!userId || targetUserId) return res.status(400).json({ error: "User id not found" });
